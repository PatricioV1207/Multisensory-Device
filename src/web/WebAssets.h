...
el('sd').innerHTML=health(s.sd.mounted,s.sd.last_write_ok?'Montada · escritura OK':'Montada · esperando','No disponible');el('queue').textContent=`Cola offline: ${s.offline.queued} · ${s.offline.bytes} B · replay ${s.offline.replayed} · pérdidas ${s.offline.dropped}`;
...
el('uptime').textContent=Math.floor(s.uptime_ms/1000)+' s';el('time').textContent='Fuente UTC: '+(s.time_source||'none');if(t.acoustic_alert_active){el('alerts').textContent=`${t.acoustic_alert_type} · ${(t.acoustic_alert_confidence*100).toFixed(0)}% · ${Math.floor(t.acoustic_alert_duration_ms/1000)} s`; }else{el('alerts').textContent=s.alerts_available?'Sin alertas acústicas activas.':'Motor de alertas no disponible.';}
